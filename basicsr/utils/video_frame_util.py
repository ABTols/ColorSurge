...
def tensor_video_lab2rgb(labs, illuminant="D65", observer="2"):
    """
    Args:
        labs    : (B, T, C, H, W) - the input tensor in LAB format
    Returns:
        tuple   : (B, T, C, H, W) - the output tensor in RGB format
    """
    illuminants = {
        "A": {'2': (1.098466069456375, 1, 0.3558228003436005),
              '10': (1.111420406956693, 1, 0.3519978321919493)},
    ...
    B, T, C, H, W = labs.shape
    
    # Permute and reshape the LAB tensor to prepare for conversion
    arrs = labs.permute((0, 1, 3, 4, 2)).contiguous()  # (B, T, C, H, W) -> (B, T, H, W, 3)
    
    L, a, b = arrs[..., 0:1], arrs[..., 1:2], arrs[..., 2:]  # Shape: (B, T, H, W, 1), (B, T, H, W, 1), (B, T, H, W, 1)
    ...
    xyz = torch.cat([x, y, z], dim=-1)  # Shape: (B, T, H, W, 3)

    mask = xyz > 0.2068966
    mask_xyz = torch.where(mask, xyz ** 3, (xyz - 16.0 / 116.) / 7.787)

    # Apply the illuminant reference white
    xyz_ref_white = torch.tensor(illuminants[illuminant][observer], device=labs.device).float()  # Shape: (3,)
    for i in range(3):
        mask_xyz[..., i] *= xyz_ref_white[i]

    # RGB conversion
    mask_xyz_reshaped = mask_xyz.view(-1, 3)  # Reshape to (B*T*H*W, 3)
    rgb_trans = torch.mm(mask_xyz_reshaped, torch.from_numpy(rgb_from_xyz).to(labs.device).type_as(mask_xyz)).view(B, T, H, W, C)
    
    # Permute back to (B, T, C, H, W)
    rgb = rgb_trans.permute((0, 1, 4, 2, 3)).contiguous()  # Shape: (B, T, C, H, W)

    # Non-linear transform
    mask = rgb > 0.0031308
    mask_rgb = torch.where(mask, 1.055 * (rgb ** (1 / 2.4)) - 0.055, rgb * 12.92)

    # Clamp RGB values to [0, 1]
    mask_rgb = torch.clamp(mask_rgb, 0, 1)

    return mask_rgb  # Shape: (B, T, C, H, W)