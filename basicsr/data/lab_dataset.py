...
class LabDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # 在 LAB 颜色空间中，L 通道表示图像的亮度，
        # 范围通常在 0 到 100 之间，其中 0 表示黑色，100 表示白色。
        # img_ab 包含图像的色彩信息，
        # 包括 a（绿色到红色）和 b（蓝色到黄色）通道。
        # 该通道的值范围通常为 [-128, 127]。
        ...
        return_d = {
            'lq': img_l, # [0,100], torch.Size([1, 256, 256])
            'gt': img_ab, # [-128, 127], shape: torch.Size([2, 256, 256])
            'target_a': target_a, # [0,64] shape: torch.Size([256, 256])
            'target_b': target_b, # [0,64] shape: torch.Size([256, 256])
            'lq_path': gt_path,
            'gt_path': gt_path
        }
        return return_d
